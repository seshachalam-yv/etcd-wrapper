...
func TestApplyPeerSkipClientSANVerify(t *testing.T) {
	...
	table := []struct {
		...
		{
			description:         "etcd 3.5+: skip-client-san-verification true inside peer-transport-security",
			configYAML:          "name: test\npeer-transport-security:\n  cert-file: /tmp/peer.crt\n  skip-client-san-verification: true\n",
			expectSkipSANVerify: true,
		},
		{
			description:         "etcd 3.5+: skip-client-san-verification false leaves PeerTLSInfo unchanged",
			configYAML:          "name: test\npeer-transport-security:\n  skip-client-san-verification: false\n",
			expectSkipSANVerify: false,
		},
		{
			description:         "etcd 3.4: top-level experimental-peer-skip-client-san-verification true",
			configYAML:          "name: test\nexperimental-peer-skip-client-san-verification: true\n",
			expectSkipSANVerify: true,
		},
		{
			description:         "etcd 3.4: top-level experimental flag false leaves PeerTLSInfo unchanged",
			configYAML:          "name: test\nexperimental-peer-skip-client-san-verification: false\n",
			expectSkipSANVerify: false,
		},
		{
			description:         "both fields unset: SkipClientSANVerify defaults to false",
			configYAML:          "name: test\n",
			expectSkipSANVerify: false,
		},
		{
			description:         "both fields false: SkipClientSANVerify stays false",
			configYAML:          "name: test\npeer-transport-security:\n  skip-client-san-verification: false\nexperimental-peer-skip-client-san-verification: false\n",
			expectSkipSANVerify: false,
		},
		{
			description:         "peer-transport-security present without skip flag: SkipClientSANVerify stays false",
			configYAML:          "name: test\npeer-transport-security:\n  cert-file: /etc/etcd/peer/tls.crt\n  key-file: /etc/etcd/peer/tls.key\n",
			expectSkipSANVerify: false,
		},
		{
			description:         "both fields true: OR resolves to true",
			configYAML:          "name: test\npeer-transport-security:\n  skip-client-san-verification: true\nexperimental-peer-skip-client-san-verification: true\n",
			expectSkipSANVerify: true,
		},
	}
...
func TestApplyPeerSkipClientSANVerifyNeverLowers(t *testing.T) {
	g := NewWithT(t)
	logger := zaptest.NewLogger(t)
	testDir := createTestDir(t)
	defer deleteTestDir(t, testDir)
	...
	configFilePath := filepath.Join(testDir, "etcd.conf.yaml")
	err := os.WriteFile(configFilePath, []byte("name: test\n"), 0600)
	g.Expect(err).ToNot(HaveOccurred())
	...
	err = applyPeerSkipClientSANVerify(configFilePath, cfg, logger)
	g.Expect(err).ToNot(HaveOccurred())
	g.Expect(cfg.PeerTLSInfo.SkipClientSANVerify).To(BeTrue())
}
